...
ship_type, ship_type_c, Indicator, Indicator_c, statement = "", "", "", "", ""
...
rules = " "*34+"GAME RULES:\nYour placed ships are on the left screen where it\
                says PLAYER. The other board on the right is your display.\n" \
               "When placing your ships type where you want to place when e.g.\
                 A1 A2 A3 A4 A5\n" \
               "Each ships is different length and the length is told by the \
                number of hole e.g. 5 holes\n" \
               "If you need want you can type [rules] when taking a shot to\
                 show the game rules\n" \
...
def draw_board():
    ...
    for x in range(9):
        print(" "*5+(" "+"-"*23)+" "*22+("-"*23))
        print(" "*5+str(x+1)+("|" +
              "|".join(game_player[x]) + "|")+" "*21+str(x+1)+("|" +
              "|".join(game_player_2[x]) + "|"))
    print(" "*5+(" "+"-"*23)+" "*22+("-"*23)+"\n")


def check_placing(n, b, board):
    global allow_ship
    if board == game_player:
        n = [a-1 for a in n]
        b = [a-1 for a in b]
        # Checks if you can place a ship there (There's
        #  no ship in that place and ship can't be close to another ship)
    for x, y_1 in zip(n, b):
        try:
            if {board[x][y_1] == board[x+1][y_1] == board[abs(x-1)][y_1] ==
               board[x][y_1+1] == board[x][abs(y_1-1)] == " "}:
                allow_ship = True
            else:
                allow_ship = False
                break
            ...
            try:
                if {board[x][y_1+1] == board[abs(x-1)][y_1] ==
                   board[x][abs(y_1-1)] == " "}:
                    allow_ship = True
                else:
                    allow_ship = False
                ...
                try:
                    if board[x][abs(y_1-1)] == board[abs(x-1)][y_1] == " ":
                        allow_ship = True
                    else:
                        allow_ship = False
                        break
...
def place_ship(size):
    ...
    while placing_ship:
        if size == 5:
            ...
            ship = input("Say where you want\
                 to put your 'Battleship'(4 holes): ")
            ship_type = "B"
        elif size == 3.5:
            ship = input("Say where you want \
                to put your 'Cruiser'(3 holes): ")
            ship_type = "c"
        elif size == 3:
            ship = input("Say where you want\
                 to put your 'Submarine'(3 holes): ")
            ship_type = "S"
        elif size == 2:
            ship = input("Say where you want to\
                 put your 'Destroyer'(2 holes): ")
            ship_type = "D"
        ship_a = ["".join(x) for x in ship]
        if ord(ship_a[0]) > 107 or ord(ship_a[len(ship_a)-2]) > 107:
        ...
        if len(numbers) != int(size):  # Checks if size of the ship is correct
            ...
            continue
        numbers_n = [abs(x) for x in numpy.diff(numbers)]
        ...
        for z in ship_b:
            if z == " ":  # Makes sure empty string doesn't go to a loop
                continue
            # Check if the column or row is the same (Same letter or number)
            elif letters[1:] == letters[:-1] or numbers[1:] == numbers[:-1]:
                # Checks if the differences between the numbers
                # or letters is 1 or -1
                if {numbers_n[1:] == numbers_n[:-1] ==
                   [1 for _ in range(len(numbers_n)-1)] or letters_n[1:]
                   == letters_n[:-1] == [1 for _ in range(len(letters_n)-1)]}:
                    if allow_ship:  # Checks if no ship placed
                        {game_player[int(next(ship_b))-1]
                         [ord(z.lower()) - 97] != ship_type}
                    else:
...
def computer_ship(size):
    ...
    if size == 5:
        ...
        ship_type_c = "D"
    while placing_ship:
        # Randomizes if a ship is horizontal or vertical
        if random.randint(1, 2) % 2 == 0:

            x = random.randint(0, 11 - int(size))
            y = random.randint(0, 8)
            letters = list(range(x, x+int(size)))
            numbers = [random.randint(y, y) for _ in range(int(size))]
            row = numbers[0]
            ...
            for z in letters:
                if allow_ship:
                    ...
                    break
                # If the ship can't be there it goes back
                # to line 144(Vertical or horizontal) and randomizes again
...
def player_shoot():
    ...
    while playing:
        ...
        if not Victory:
            ...
            break
        # Checks if the player hits and prints only hit if player hits
        if Indicator == " "*40+"It's a hit!" and not same_shot:
        ...
        shot = list(input("Say where you want to shoot: "))
        if "".join(shot).lower() == "rules":
            print(rules)
            continue
            ...
            input("Press any key to close")
            break
        # Makes sure empty string doesn't go to a shot (A 5 makes A5)
        shot_a = [x for x in shot if x != " "]
        shot_b = iter(shot_a)
